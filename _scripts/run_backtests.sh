    ...
    for curr_year in ${years[@]}; do
      for curr_bt_source in ${bt_sources[@]}; do
        for curr_spread in ${spreads[@]}; do
          ...
          run_backtest.sh -m 1 -v -t -e $name -f "$dir/$setfile" -c $currency -p $pair -d $curr_deposit -y $curr_year -s $curr_spread -b $curr_bt_source -r "$report_name" -D "$dir/_test_results"
        done
      done
    done
  done
