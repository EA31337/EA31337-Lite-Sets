...
find "$ROOT" -type f -name "test.ini" -print0 | sort -z | while IFS= read -r -d '' file; do
  ...
  find "$ROOT" -type f '(' -name "*$1*.rule" -o -name "*$1*.rules" ')' -print0 | while IFS= read -r -d '' rule_file; do
    ...
    run_backtest.sh ${@:2} -v -o -t -e $name -f "$dir/$setfile" -c $currency -p $pair -d $deposit -D $digits -y $year -s $spread -b $bt_source -r "$report_name" -i "$RULES_INIT" -i "$rule_file" -O "$dir/_optimization_results"
    [ "$TEST" ] && continue
    # gen_report.sh "$dir/_optimization_results" "$OUT"
    [ "$PUSH" ] && push_report.sh "Optimization-$report_base" "Optimization results: $report_name"
  done
done
echo "$0 done."
